...
export class BaseCrudService<T> {
  ...
  public create(record): Observable<T> {
  ...
  public update(record): Observable<T> {

    return this.http.put<T>(`${this.API_URL}/${record['uuid']}`, record, {
      headers: new HttpHeaders().set('X-Requested-With', 'XMLHttpRequest')
    })
    .pipe(
      tap(response => console.log(response)),
  ...
  deleteById(uuid: string): Observable<T> {
    return this.http.delete<T>(`${this.API_URL}/${uuid}`)
    .pipe(
      take(1),
      catchError((err: any) => {
        return throwError(`Fail on delete ${this.className} by ID ${uuid}!`);    //Rethrow it back to component
      })
    );
  }
